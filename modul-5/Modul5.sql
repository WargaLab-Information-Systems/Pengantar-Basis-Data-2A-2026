...
SELECT m.nim, m.nama, mk.nama_mk, SUM(mk.sks) AS Total_SKS
FROM mahasiswa m
JOIN krs k ON m.nim = k.nim
JOIN mata_kuliah mk ON k.kode_mk = mk.kode_mk
WHERE mk.kode_mk = "MK03"
GROUP BY m.nim, m.nama;



SELECT m.nama, k.nim, SUM(mk.sks) AS Total_SKS
FROM krs k
JOIN mata_kuliah mk ON k.kode_mk = mk.kode_mk
...
SELECT m.nama, m.angkatan, mk.sks, mk.nama_mk, d.nama_dosen, n.nilai_angka
FROM mahasiswa m
JOIN nilai n ON m.nim = n.nim 
JOIN mata_kuliah mk ON n.kode_mk = mk.kode_mk 
JOIN dosen d ON mk.id_dosen = d.id_dosen 
WHERE n.nilai_angka > 80 AND d.id_dosen IN (1, 3, 5) AND m.jurusan = 'Sistem Informasi' AND m.angkatan > 2024;